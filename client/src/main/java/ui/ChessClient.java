...
public class ChessClient implements ServerMessageObserver {
  ...
  private State uiState = State.PRE_LOGIN;
  ...
  private void eval(String... args) throws UIException, ResponseException {
    ...
    switch (uiState) {
      case PRE_LOGIN -> new PreLoginMenu().evalPreLogin(args);
      case POST_LOGIN -> new PostLoginMenu().evalPostLogin(args);
      case GAMEPLAY -> new GameplayMenu().evalGameplay(args);
    }
  }

  // PRE LOGIN
  private class PreLoginMenu {
    public void evalPreLogin(String... args) throws UIException, ResponseException {
      String cmd = args[0];
      switch (cmd) {
        case "register" -> {
  ...
  private class PostLoginMenu {
    ...
    private void joinGame(Integer gameID, String teamColorString) throws ResponseException {
      teamColor = teamColorString.equals("white") ? TeamColor.WHITE : TeamColor.BLACK;
      actualGameID = displayGameIDsToRealGameIDs.get(gameID);
